...
const rows = [
  ...
  R('V_AMR', 'فِعْلُ أَمْرٍ مَبْنِيٌّ عَلَى السُّكُونِ', CH.afaal, '70–72', 'أقسام الأفعال', 'فِعْلُ أَمْرٍ مَبْنِيٌّ عَلَى السُّكُونِ'),
  R('V_MUJARRAD', 'مَرْفُوعٌ لِتَجَرُّدِهِ مِنَ النَّاصِبِ وَالْجَازِمِ', CH.afaal, '73,95', 'رفع المضارع', 'لِتَجَرُّدِهِ مِنَ النَّاصِبِ وَالْجَازِمِ'),
  ...
  /* The `requires` discriminator is NEW, and it exists because the muḍmar row below now renders the
     same term. Without it this row could be credited by the component card of «نُصِحْتُ» alone, so
     a break in the ẓāhir lane would go unnoticed — the exact collision B_ZARF_THAMMA was found in.
     «مَرْفُوعٌ» is what only the ẓāhir card says: the muḍmar deputy is a built pronoun with a maḥall
     and no state, so its card can never carry the word. */
  R('M_NAIB_FAIL', 'نَائِبُ فَاعِلٍ', CH.naib, '96–98', 'المرفوعات', 'نَائِبُ فَاعِلٍ',
    { requires: 'مَرْفُوعٌ' }),
  /* p. 97's second half, shipped. The PROBE is «نَائِبُ فَاعِلٍ» rather than the row's own term,
     for exactly the reason B_MAFUL_MUDMAR_MUTTASIL's is «مَفْعُولٌ بِهِ»: «مضمر» is how the book
     CLASSIFIES the deputy, not a word anyone utters while parsing. What a learner actually says of
     «نُصِحْتُ» is «التاء: ضمير متصل مبني على الضم في محل رفع نائب فاعل», so the `requires`
     discriminator is the attached-pronoun class — which the ẓāhir deputy's card («نَائِبُ فَاعِلٍ
     مَرْفُوعٌ …») never carries, so the two halves of the chapter cannot answer for each other. */
  R('M_NAIB_MUDMAR', 'نَائِبُ فَاعِلٍ مُضْمَرٌ', CH.naib, '97–98', 'نائب الفاعل', 'نَائِبُ فَاعِلٍ',
    { requires: 'ضَمِيرٌ مُتَّصِلٌ' }),
  ...
  /* The old probe «مَفْعُولٌ بِهِ مُتَّصِلٌ» was a FALSE ABSENT: it welds two of the app's labels
     together in an order no card ever renders. The attached object pronoun HAS been produced since
     Phase 0 — p. 157's «لَقِيَتْنِي» yāʾ — and the card reads «… ضَمِيرٌ مُتَّصِلٌ مَبْنِيٌّ عَلَى
     السُّكُونِ فِي مَحَلِّ نَصْبٍ مَفْعُولٌ بِهِ», with the class BEFORE the bināʾ and the role after
     it. Both halves are now asserted separately, and both are composed from the app's own frozen
     labels by work/w15-probes.js rather than retyped. */
  R('B_MAFUL_MUDMAR_MUTTASIL', 'مَفْعُولٌ بِهِ مُضْمَرٌ مُتَّصِلٌ', CH.mafulBih, '142–145,157', 'أقسام المفعول به', 'مَفْعُولٌ بِهِ',
    { requires: 'ضَمِيرٌ مُتَّصِلٌ' }),
  R('B_MAFUL_MUDMAR_MUNFASIL', 'مَفْعُولٌ بِهِ مُضْمَرٌ مُنْفَصِلٌ', CH.mafulBih, '142–145', 'أقسام المفعول به', 'إِيَّاكَ',
    { trueBlocker: "TRUE BLOCKER under the approved source, and it is the SAME missing fact as ضَمِيرٌ مُنْفَصِلٌ and مُبْتَدَأٌ مُضْمَرٌ — the bināʾ of the detached pronouns — reached from a third bāb. (1) Al-Tuḥfah teaches the row in full: p. 142 divides المفعول به into ظاهر and مضمر and the مضمر into متصل and منفصل, and p. 143 enumerates the twelve منفصل forms — «وَلِلْمُنْفَصِلِ اثْنَا عَشَرَ لَفْظاً أَيْضاً: «إِيَّا» مُرْدَفَةً بِالْيَاءِ لِلْمُتَكَلِّمِ وَحْدَهُ، أَوْ «نَا» لِلْمُعَظِّمِ نَفْسَهُ، أَوْ مَعَ غَيْرِهِ، أَوْ بِالْكَافِ مَفْتُوحَةً لِلْمُخَاطَبِ الْمُفْرَدِ الْمُذَكَّرِ …» — settles the disputed analysis in its own voice («وَالصَّحِيحُ أَنَّ الضَّمِيرَ هُوَ «إِيَّا» وَأَنَّ مَا بَعْدَهُ لَوَاحِقُ تَدُلُّ عَلَى التَّكَلُّمِ أَوِ الْخِطَابِ أَوِ الْغَيْبَةِ»), supplies complete host sentences («إِيَّايَ أَطَاعَ التَّلَامِيذُ», «مَا أَطَاعَ التَّلَامِيذَ إِلَّا إِيَّايَ») and cites ﴿إِيَّاكَ نَعْبُدُ وَإِيَّاكَ نَسْتَعِينُ﴾. It is emphatically NOT sourceExcluded, and «مفعول به» is a direct analysis, so the row stays counted. (2) It is never PARSED. p. 143 gives the list and stops; p. 144's تمرينات set it as unanswered student work (ex. 1 «ضع ضميراً منفصلاً … ليكون مفعولاً به», ex. 3 «إياهما، إياكم، إيّاي …», ex. 6); p. 145's أسئلة likewise asks the learner to supply the examples («مَثِّلْ بِثَلَاثَةِ أَمْثِلَةٍ لِلْمُضْمَرِ الْمُنْفَصِلِ الْوَاقِعِ مَفْعُولاً بِهِ») and its أعرب exercise carries no answer key. Every object pronoun this book DOES parse is attached: the yāʾ of «لَقِيَتْنِي» at p. 157 («وَالْيَاءُ ضَمِيرُ الْمُتَكَلِّمِ مَفْعُولٌ بِهِ، مَبْنِيٌّ عَلَى السُّكُونِ فِي مَحَلِّ نَصْبٍ») and the نا of «سَيَزُورُنَا» at p. 95 («وَنَا: مَفْعُولٌ بِهِ مَبْنِيٌّ عَلَى السُّكُونِ فِي مَحَلِّ نَصْبٍ») — and this app produces both. (3) إِيَّاكَ is a standalone word, so it is necessarily a BUILT noun here — isBuiltNoun(): a noun carrying a maḥall and no state — and builtNounBinaaMarker() admits a built noun only through a per-construction, source-verified bināʾ table. p. 22 is this book's own catalogue of what is built and upon what («وَذَلِكَ كَلُزُومِ «كَمْ» وَ«مَنْ» السُّكُونَ، وَكَلُزُومِ «هٰؤُلَاءِ» وَ«حَذَامِ» وَ«أَمْسِ» الْكَسْرَ، وَكَلُزُومِ «مُنْذُ» وَ«حَيْثُ» الضَّمَّ، وَكَلُزُومِ «أَيْنَ» وَ«كَيْفَ» الْفَتْحَ») and no pronoun stands in it. Rendering «إِيَّاكَ: ضَمِيرٌ مُنْفَصِلٌ مَبْنِيٌّ عَلَى الْفَتْحِ فِي مَحَلِّ نَصْبٍ مَفْعُولٌ بِهِ» would therefore mean INVENTING the bināʾ sign, which is refused here for the same reason it is refused for the other two rows. NOT the obstacle: the host, which p. 143 supplies whole. UNBLOCK: one reviewed secondary source for the bināʾ of the detached pronouns, registered through secondarySupportedRule + REVIEWED_SOURCE_AUTHORITIES exactly as أَلِفُ الِاثْنَيْنِ is handled today. ONE such citation unblocks this row, ضَمِيرٌ مُنْفَصِلٌ and مُبْتَدَأٌ مُضْمَرٌ together, because it is one missing fact serving three bābs. It must be read and verified by a human against the secondary text; it is deliberately NOT invented here." }),
  ...
  /* FINAL MARATHON. The `partial` flag this row used to carry was measuring LEXICAL coverage of
     p. 149–150's twelve ألفاظ — which is not what this inventory counts anywhere else, and not
     even what its own sibling is held to. B_ZARF_MAKAN registers six of p. 152's THIRTEEN place
     ألفاظ (أَمَامَ، خَلْفَ، فَوْقَ، تَحْتَ، عِنْدَ، مَعَ) and is FULL; this row registers six of
     p. 149–150's twelve (الْيَوْمَ، لَيْلَةً، سَحَرًا، صَبَاحًا، مَسَاءً، حِينًا) plus two muḍāf
     forms, and was PARTIAL. The ratios are the same; only the flag differed. No ẓarf LEXEME has a
     row of its own in this inventory except the two built ones below, and those are rows because
     of a bināʾ claim rather than because they are lexemes. The TERM «ظرف زمان» is uttered in full
     — «مَسَاءً: مَفْعُولٌ فِيهِ ظَرْفُ زَمَانٍ مَنْصُوبٌ، وَعَلَامَةُ نَصْبِهِ الْفَتْحَةُ
     الظَّاهِرَةُ عَلَى آخِرِهِ» — so the row is FULL. The six unproduced ألفاظ stay named in
     GRAMMAR_COVERAGE_MATRIX.deliberatelyNotGenerated, which is where a coverage fact belongs. */
  ...
  /* FINAL MARATHON — the two built place ẓarfs, and a FALSE FULL corrected. B_ZARF_THAMMA was
     scored FULL by a skeleton collision: skeleton(«ثَمَّ») is "ثم", which is also skeleton(«ثُمَّ»),
     the ḥarf ʿaṭf this app does produce. The card-mode probe carried no `requires`, so the ʿaṭf
     particle's own card answered for the ẓarf. Exact-byte occurrences of «ثَمَّ» in any rendered
     card: zero. Both rows now carry the place-ẓarf discriminator, written from the app's own
     MAFUL_FIH_KIND_LABELS by work/w15-zarf-probes.js, and both are TRUE_BLOCKERs. */
  R('B_ZARF_THAMMA', 'ثَمَّ', CH.zarf, '151–152', 'ظرف المكان المبني', 'ثَمَّ',
    { mode: 'card', requires: 'ظَرْفُ مَكَانٍ',
      trueBlocker: "TRUE BLOCKER under the approved source, and the obstacle is the bināʾ — the same wall as the detached pronouns, reached from باب ظرف الزمان وظرف المكان. (1) Al-Tuḥfah TEACHES the word. p. 151's matn puts it in the closed list of place ẓarfs — «وَظَرْفُ الْمَكَانِ هُوَ: اسْمُ الْمَكَانِ الْمَنْصُوبُ بِتَقْدِيرِ «فِي»، نَحْوَ: أَمَامَ، وَخَلْفَ، وَقُدَّامَ، وَوَرَاءَ، وَفَوْقَ، وَتَحْتَ، وَعِنْدَ، وَإِزَاءَ، وَحِذَاءَ، وَتِلْقَاءَ، وَثَمَّ، وَهُنَا، وَمَا أَشْبَهَ ذَلِكَ» — and p. 152's شرح gives it its own numbered entry with a Qurʾānic shāhid: «وَالثَّانِي عَشَرَ: «ثَمَّ» نَحْوَ قَوْلِ اللهِ تَعَالَى: ﴿وَأَزْلَفْنَا ثَمَّ الْآخَرِينَ﴾». So it is neither sourceExcluded nor notCounted: a learner meeting that verse must parse the word, and the analysis is a real utterance in the shape «ثَمَّ: مَفْعُولٌ فِيهِ ظَرْفُ مَكَانٍ …». (2) What the source WITHHOLDS is what the word does at its end. ثَمَّ is indeclinable, so in this engine it is necessarily a BUILT noun — isBuiltNoun(): a noun carrying a maḥall and no state — and builtNounBinaaMarker() admits a built noun only through a per-construction, source-verified bināʾ table. p. 22 is this book's own catalogue of what is built and upon what: «وَذَلِكَ كَلُزُومِ «كَمْ» وَ«مَنْ» السُّكُونَ، وَكَلُزُومِ «هٰؤُلَاءِ» وَ«حَذَامِ» وَ«أَمْسِ» الْكَسْرَ، وَكَلُزُومِ «مُنْذُ» وَ«حَيْثُ» الضَّمَّ، وَكَلُزُومِ «أَيْنَ» وَ«كَيْفَ» الْفَتْحَ». ثَمَّ is not in it, and the bāb itself never says the word is مبني or gives it a marker. (3) The one clause that looks like an escape is not one. p. 151's matn calls the WHOLE list «الْمَنْصُوبُ», with no exception for ثَمَّ and هُنَا, so a literal reading would license «ثَمَّ: مَفْعُولٌ فِيهِ ظَرْفُ مَكَانٍ مَنْصُوبٌ وَعَلَامَةُ نَصْبِهِ الْفَتْحَةُ الظَّاهِرَةُ». That was refused: it asserts an iʿrāb SIGN on a word whose final fatḥah is a bināʾ marker, which is the built-vs-muʿrab contract Wave 12 strengthened globally and which p. 22's own definition forbids («وَالْمَبْنِيُّ: مَا لَزِمَ آخِرُهُ حَالَةً وَاحِدَةً لِغَيْرِ عَامِلٍ وَلَا اعْتِلَالٍ»). Inventing «مَبْنِيٌّ عَلَى الْفَتْحِ» instead is the other refused move. GRAMMAR_COVERAGE_MATRIX has recorded the same finding all along — «the mabnī place ẓarfs ثَمَّ and هُنَا, which the noun sign matrix does not model» — but recorded it as a scope decision; it is a source limit. UNBLOCK: one reviewed secondary source for the bināʾ of ثَمَّ, registered through secondarySupportedRule + REVIEWED_SOURCE_AUTHORITIES exactly as أَلِفُ الِاثْنَيْنِ is handled today, read and verified by a human against that text. It is deliberately NOT invented here." }),
  R('B_ZARF_HUNA', 'هُنَا', CH.zarf, '151–152', 'ظرف المكان المبني', 'هُنَا',
    { mode: 'card', requires: 'ظَرْفُ مَكَانٍ',
      trueBlocker: "TRUE BLOCKER under the approved source, on exactly the evidence that blocks its neighbour ثَمَّ, and its old missingReason («Same deferral as ثَمَّ») was pointing at a row that was itself falsely scored FULL. (1) Al-Tuḥfah TEACHES the word: p. 151's matn lists it among the place ẓarfs — «… وَتِلْقَاءَ، وَثَمَّ، وَهُنَا، وَمَا أَشْبَهَ ذَلِكَ» — and p. 152's شرح closes its thirteen ألفاظ with it, supplying a complete host sentence of its own: «وَالثَّالِثَ عَشَرَ: «هُنَا» نَحْوَ قَوْلِكَ: «جَلَسَ مُحَمَّدٌ هُنَا لَحْظَةً»». That is a two-ẓarf sentence this engine builds in every other respect — a past ʿāmil, an explicit fāʿil, and a time ẓarf (لَحْظَةً) beside the place one — so the HOST is not the obstacle. (2) The obstacle is the bināʾ. هُنَا is indeclinable, so it is necessarily a BUILT noun here, and builtNounBinaaMarker() admits a built noun only through a per-construction, source-verified bināʾ table. p. 22's catalogue of what is built and upon what names كَمْ، مَنْ، هٰؤُلَاءِ، حَذَامِ، أَمْسِ، مُنْذُ، حَيْثُ، أَيْنَ، كَيْفَ — and not هُنَا. Note how close the book comes and still stops: it licenses هٰؤُلَاءِ, the demonstrative of persons, which is what let اسم الإشارة ship in Wave 4 and again in Wave 13's rābiṭ lane; هُنَا, the demonstrative of PLACE, it never mentions. That asymmetry is the source's, not the engine's. (3) p. 151's blanket «الْمَنْصُوبُ» over the whole list cannot be borrowed for it, for the same reason it cannot be borrowed for ثَمَّ: it would assert an iʿrāb sign on a word p. 22's own definition of المبني excludes from having one. Both available moves — claiming a sign, or inventing «مَبْنِيٌّ عَلَى السُّكُونِ» — were refused. UNBLOCK: one reviewed secondary source for the bināʾ of هُنَا, registered through secondarySupportedRule + REVIEWED_SOURCE_AUTHORITIES exactly as أَلِفُ الِاثْنَيْنِ is handled today. One citation covering the built place ẓarfs lifts this row and ثَمَّ together. It must be read and verified by a human against that text; it is deliberately NOT invented here." }),
